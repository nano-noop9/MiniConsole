...
static size_t id3v2_tag_size(const mp3_id3_header_v2_t *tag)
{
                  ...
                  (magic[2] == 0x33)) /* 'ID3' */
        {
            ...
            if (sizeof(mp3_id3_header_v2_t) == fread(&tag, 1, sizeof(mp3_id3_header_v2_t), fp)) {
                if (memcmp("ID3", (const void *) &tag, sizeof(tag.header)) == 0) {
                    is_mp3_file = true;
                    has_id3v2_tag = true;
                    id3v2_size = sizeof(mp3_id3_header_v2_t) + id3v2_tag_size(&tag);
                    if(tag.flag & 0x10)
                    {
                        id3v2_size += 10;
                    }
    ...
    if (offset >= 0) {
        ...
        if(mp3_dec_err == ERR_MP3_NONE) {
            ...
            if (pInstance->eof_reached) {
                ...
                if(pInstance->read_ptr == read_ptr)
                {
                    pInstance->read_ptr++;
                }
                return DECODE_STATUS_NO_DATA_CONTINUE;
